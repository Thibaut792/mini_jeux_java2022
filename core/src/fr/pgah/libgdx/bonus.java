...
public class bonus {
    ...
    public void initialiser(String img) {
    ...
    public boolean CollisionAvec(Souris souris) {

        if (rekt.overlaps(souris.rekt)) {
            return true;
        } else {
            return false;
        }
    ...
    public void dessiner(SpriteBatch batch) {
        batch.draw(img, coordX, coordY, longueurEffective / 2, hauteurEffective / 2, longueurEffective,
                hauteurEffective, 1, 1, rotation, 0, 0, img.getWidth(), img.getHeight(), false, false);
    }
}
