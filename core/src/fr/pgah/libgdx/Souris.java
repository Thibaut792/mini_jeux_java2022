...
public class Souris {
  ...
  public void dessiner(SpriteBatch batch) { // Les variables local sont supprimés dés que l'on sort de la méthode
                                            // ici c'est la copie de la rérérence a l'objet batch qui disparait mais la
                                            // réference continue a éxister dans la class principal
  ...
  public boolean estEnCollisionAvec(ArrayList<Sprite> sprites) {
    ...
    // sinon faux
    // boucle for reach = Sprite = le type de tableaux, sprites c'est le nom du
    // tableau que l'on va parcourrir, sprite est une variable
